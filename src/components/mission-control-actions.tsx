...
type TaskType = "qualification.evaluate" | "outreach.prepare" | "analytics.snapshot";
...
export function MissionControlActions({ prospects }: { prospects: Array<{ id: string; label: string }> }) {
  ...
  const needsProspect = taskType !== "analytics.snapshot";

  async function queueTask() {
    setBusy(true);
    setMessage("");
    const task = needsProspect ? { type: taskType, prospectId: prospectId.trim() } : { type: taskType };
    ...
    <section className="card mt-8 p-6">
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Tasks run only when you queue and execute them. Outreach creates an internal draft for review; it never sends email.</p>
        </div>
      ...
      <div className="mt-5 grid gap-3 lg:grid-cols-[1fr_1fr_1fr_auto_auto]">
        ...
        <div>
          <input list="mission-control-prospects" value={prospectId} onChange={(event) => setProspectId(event.target.value)} disabled={!needsProspect} placeholder={needsProspect ? "Select or paste prospect ID" : "No prospect required"} className="w-full rounded-xl border border-slate-300 px-4 py-3 text-sm disabled:bg-slate-100" />
        ...
        <button type="button" onClick={queueTask} disabled={busy || (needsProspect && !prospectId.trim())} className="rounded-xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white disabled:opacity-50">Queue task</button>
