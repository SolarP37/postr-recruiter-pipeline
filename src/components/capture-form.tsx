...
export function CaptureForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage(`${result.created} prospect record${result.created === 1 ? "" : "s"} created for review.`);
    router.refresh();
    if (result.prospectIds?.[0]) router.push(`/prospects?selected=${result.prospectIds[0]}`);
  }

  return (
    <form onSubmit={submit} className="grid gap-6 lg:grid-cols-[1fr_0.85fr]">
      <div className="card p-6">
        <label className="field-label">Platform
          <select name="platform" className="field" required defaultValue="instagram">
        ...
        <label className="field-label mt-5">Public profile URL
          <input name="sourceUrl" className="field" type="url" placeholder="https://…" />
        </label>
        <label className="field-label mt-5">Campaign
          <input name="campaign" className="field" maxLength={120} placeholder="Optional campaign label" />
        </label>
        ...
        <button disabled={busy} className="button-primary mt-6 w-full disabled:opacity-50" type="submit">
      ...
      <div className="card flex min-h-96 items-center justify-center overflow-hidden p-4">
        {preview ? <Image src={preview} alt="Screenshot preview" width={900} height={1200} unoptimized className="max-h-[42rem] w-auto rounded-xl object-contain" /> : <p className="px-8 text-center text-sm text-slate-400">Screenshot preview appears here.</p>}
      </div>
    </form>
  );
}
