...
export const missionControlTaskSchema = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("brand.discovery.search"), query: z.string().trim().min(3).max(300), limit: z.number().int().min(1).max(10).default(5) }),
  ...
  z.object({ type: z.literal("autonomy.evaluate"), messageId: z.string().min(1).max(100), action: z.enum(["CREATE_GMAIL_DRAFT", "SEND_INITIAL", "SEND_FOLLOW_UP"]) }),
...
export const taskAgentIds: Record<MissionControlTaskRequest["type"], string> = {
  ...
  "brand.discovery.search": "brand-discovery",
  ...
  "autonomy.evaluate": "email-generation",
